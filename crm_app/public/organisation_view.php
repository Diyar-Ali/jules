    ...
    <?php if ($org_data): ?>
        <div class="card">
            <div class="card-header">
                <h3><?php echo htmlspecialchars($org_data['name']); ?>
            ...
            <div class="card-body">
                <p><strong>Website:</strong> <?php echo $org_data['website'] ? '<a href="'.htmlspecialchars($org_data['website']).'" target="_blank">'.htmlspecialchars($org_data['website']).'</a>' : 'N/A'; ?></p>
                <p><strong>Phone:</strong> <?php echo htmlspecialchars($org_data['phone'] ?? 'N/A'); ?></p>
                <p><strong>Industry:</strong> <?php echo htmlspecialchars($org_data['industry'] ?? 'N/A'); ?></p>
                <p><strong>Annual Revenue:</strong> <?php echo $org_data['annual_revenue'] ? number_format($org_data['annual_revenue'], 2) : 'N/A'; ?></p>

                <h4>Address</h4>
                <p>
                    <?php echo htmlspecialchars($org_data['address_street'] ?? ''); ?><br>
                    <?php echo htmlspecialchars($org_data['address_city'] ?? ''); ?><?php echo $org_data['address_state'] ? ', '.htmlspecialchars($org_data['address_state']) : ''; ?> <?php echo htmlspecialchars($org_data['address_zip'] ?? ''); ?><br>
                    <?php echo htmlspecialchars($org_data['address_country'] ?? ''); ?>
                </p>

                <h4>Description</h4>
                <p><?php echo nl2br(htmlspecialchars($org_data['description'] ?? 'N/A')); ?></p>

                <h4>Audit Information</h4>
                <p><strong>Created By:</strong> <?php echo htmlspecialchars($org_data['created_by_username'] ?? 'Unknown'); ?></p>
                <p><strong>Created At:</strong> <?php echo date("Y-m-d H:i:s", strtotime($org_data['created_at'])); ?></p>
                <p><strong>Last Updated At:</strong> <?php echo date("Y-m-d H:i:s", strtotime($org_data['updated_at'])); ?></p>
                <p><strong>Version:</strong> <?php echo htmlspecialchars($org_data['version']); ?></p>
            </div>
            <div class="card-footer">
                <a href="organisation_edit.php?id=<?php echo $org_data['id']; ?>" class="btn btn-warning">Edit</a>
                ...
                <form action="organisations.php" method="POST" style="display: inline;">
                    ...
                    <input type="hidden" name="organisation_id" value="<?php echo $org_data['id']; ?>">
                    <input type="hidden" name="version" value="<?php echo $org_data['version']; ?>">
