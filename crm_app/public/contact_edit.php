...
<div class="container">
    ...
    <form action="contact_edit.php<?php echo $edit_mode ? '?id='.$contact_data['id'] : (isset($_GET['organisation_id']) ? '?organisation_id='.$_GET['organisation_id'] : ''); ?>" method="POST">
        <?php echo csrf_input_field(); ?>
        <?php if ($edit_mode): ?>
            <input type="hidden" name="id" value="<?php echo htmlspecialchars($contact_data['id']); ?>">
            <input type="hidden" name="version" value="<?php echo htmlspecialchars($contact_data['version']); ?>">
        <?php endif; ?>

        <div class="row">
            <div class="col-md-6 mb-3">
                <label for="first_name" class="form-label">First Name <span class="text-danger">*</span></label>
                <input type="text" class="form-control" id="first_name" name="first_name" value="<?php echo htmlspecialchars($contact_data['first_name']); ?>" required>
            </div>
            <div class="col-md-6 mb-3">
                <label for="last_name" class="form-label">Last Name <span class="text-danger">*</span></label>
                <input type="text" class="form-control" id="last_name" name="last_name" value="<?php echo htmlspecialchars($contact_data['last_name']); ?>" required>
            </div>
        </div>
        <div class="mb-3">
            <label for="email" class="form-label">Email</label>
            <input type="email" class="form-control" id="email" name="email" value="<?php echo htmlspecialchars($contact_data['email']); ?>">
        </div>
        <div class="row">
            <div class="col-md-6 mb-3">
                <label for="phone_mobile" class="form-label">Mobile Phone</label>
                <input type="tel" class="form-control" id="phone_mobile" name="phone_mobile" value="<?php echo htmlspecialchars($contact_data['phone_mobile']); ?>">
            </div>
            <div class="col-md-6 mb-3">
                <label for="phone_work" class="form-label">Work Phone</label>
                <input type="tel" class="form-control" id="phone_work" name="phone_work" value="<?php echo htmlspecialchars($contact_data['phone_work']); ?>">
            </div>
        </div>
        <div class="mb-3">
            <label for="title" class="form-label">Title/Position</label>
            <input type="text" class="form-control" id="title" name="title" value="<?php echo htmlspecialchars($contact_data['title']); ?>">
        </div>
        <div class="mb-3">
            ...
            <select class="form-select" id="organisation_id" name="organisation_id">
                ...
                <?php foreach ($organisations_list as $org): ?>
                    <option value="<?php echo $org['id']; ?>" <?php echo ($contact_data['organisation_id'] == $org['id']) ? 'selected' : ''; ?>>
        ...
        <a href="contacts.php<?php echo ($contact_data['organisation_id'] && !$edit_mode) ? '?organisation_id_filter='.$contact_data['organisation_id'] : ''; ?>" class="btn btn-secondary">Cancel</a>
    </form>
</div>
